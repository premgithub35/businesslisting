         ...
         <div class="col-md-3">
          ...
          <div class="col-md-3">
            <input type="text" class="form-control " name="name"  placeholder=" Enter Name" value="{{app('request')->input('name') }}">
                   ...
                   <a href="{{'message'}}" class="btn btn-danger btn-md"><span class="fa fa-remove"></span></a>
                    </span>
                 </div>   
...
<hr/>
            ...
            <thead>
                ...
                <th><a href="javascript:void(0)">contact</a></th>
                <th><a href="javascript:void(0)">Service name</a></th>
         ...
         <tbody class="text-capitalize">
          @foreach($enquirys as $i => $enquiry)
              <tr>
                  <td style="color:dimgrey"><b>{{ $enquiry->id }}</b></td>
                  <td><b> {{ $enquiry->name }}</b></td>
                  <td style="color: dimgrey" > <b>{{ $enquiry->email }}</b></td>
                 
                  <td style="color: dimgrey"><b>{{ $enquiry->mobile }}</b></td>
                  <td style="color: dimgrey"><b>{{ $enquiry->serviceitem }}</b></td>

                   <td>
                    @if ($enquiry->is_active == 0)
                ...
                @endif
                  ...
                  <td>
                    <a href="{{route('inactive-message',$enquiry->id)}}" class="btn btn-sm btn-danger"><i class="fa fa-remove"></i></a>
                    <a href="{{route('active-message',$enquiry->id)}}" class="btn btn-sm btn-success"><i class="fa fa-check-square"></i></a>
                    </td>
                  </tr>
              @endforeach
      </tbody>
